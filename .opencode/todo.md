...
### Slice 4: Non-blocking hot path | parallel-group:C | risk:H | depends:Slice1,2 | status:in_progress
...
- [ ] S4.4: MODIFY audio.rs — background std::thread for format switch (fixes B1) | file:src/audio.rs | size:M | DEFECT (verified 03:04) — audio.rs has NO std::thread::spawn (grep=0); set_output_format is synchronous, blocks up to 310ms (250ms verify_format_landed + 60ms settle L283-284,334); called synchronously from start_playback (L794)/load_track (L957)/load_remote (L996) on the input path; NO test audio_switch_does_not_block_input; AC-M9.2.4 NOT MET; see SYNC-20
- [ ] S4.5: MODIFY app.rs start_playback/load_track — gate on audio-ready signal | file:src/tui/app.rs | size:S | DEFECT (verified 03:04) — NO audio_ready/AudioReady/audio_signal anywhere in codebase (grep=0); NO gating mechanism; start_playback/load_track call set_output_format synchronously with `let _ =`; NOT implemented; see SYNC-20
...
- [ ] S4.R: Review — discover_opens_instantly + cont_youtube_auto_advance_non_blocking pass | agent:Reviewer | size:S | FAIL (verified 03:04) — 4/6 leaf PASS (S4.1/S4.2/S4.3/S4.6), 2 DEFECT (S4.4/S4.5 — never implemented, false [x] marks reverted); see SYNC-20
