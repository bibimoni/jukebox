...
use jukebox::tui::queue::{RepeatMode, ShuffleMode, Transport};
...
fn cat_with_artists() -> (tempfile::TempDir, Catalog) {
    ...
    .collect();
    let json = serde_json::json!({"version":1,"built_at":"x","source_root":"/tmp","tracks":tracks})
        .to_string();
...
fn smart_shuffle_avoids_back_to_back_same_artist() {
    ...
    let order: Vec<String> = t
        .order
        .iter()
        .map(|&i| t.context.track_ids(&R)[i].clone())
        .collect();
    ...
    for w in order.windows(2) {
        assert_ne!(
            artist_of(&cat, &w[0]),
            artist_of(&cat, &w[1]),
            "smart shuffle placed same artist back-to-back: {:?}",
            order
        );
    }
